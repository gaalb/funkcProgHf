...
2. A feladat innentől a következő: végig haladni a kiterített {row, col} koordináta párok kigöngyölt listáján, és dönteni minden esetben, hogy nullát, vagy a soron következő nemnulla számot rendelni hozzájuk, úgy hogy a beszúrt elem ne sértse a követelményeket.
3. Minden indexen tehát döntést hozunk: nulla, vagy nemnulla. Ezen döntések egy fát képeznek, és a döntési fa levelei konkrét n\*n mátrix kialakításokat jelentenek. Ezen levelek közül azok jelentenek megoldást a feladványra, amikben a pontosan megfelelő számú nulla és nemnulla elem van. Ezen döntési fának a bejárása a feladatunk, úgy, hogy azon ágakat, amelyekről korán el lehet dönteni, hogy nem vezethetnek megoldáshoz, elhagyjuk/vágjuk. 
...
defp feasible_line?(used, filled, n, m) do
 ...
 A döntési fában keresést a `dfs/14` függvény valósítja meg. Rekurzív, mindig a döntési fa egy csomópontjában hívjuk meg. A visszatérési értéke az azon csompontból kiinduló részfa megoldásainak halmaza (pontosabban listája). Mivel minden csomópontban kétféle döntés van, a megoldások listája: [megoldások ha nullát választunk] ++ [megoldások ha nem nullát választunk].
 Mindkét eset egy-egy rekurzív hívást jelent egy új csomópontból, ami úgy adódik, hogy a kérdéses helyre a kiterített listában nullát, vagy nem nullát írunk. Ennek tényét úgy adjuk át a következő rekurzív hívásnak, hogy akkumulátorba építjük a megoldást.
...
```
Tehát akkor áll le a rekurzió egy csomópontban, ha az összes elemről döntést hoztunk. Elfogadható a megoldás ha ez egy olyan kialakítást eredményezett, amiben megfelelő mind a nullás, mind a nemnulla elemek száma.
Azon csomópontokban a döntési fában, amelyek nem levelek, az általánosab klóz hívódik meg:
...
2. A `take_val`-ba teszi azon részfa eredményeit, ami úgy keletkezik, hogy nem nullát választ következő elemnek
   1. A with blokk automatikusan vág, ha: 
      ...
      - a soron következő szám ellentmondásban van a kiinduló konfigurációval
...
- `pairs_to_grid/2` elvárt tulajdonság: Az eredmény egy `n×n`-es lista-lista. A bemeneti `pairs_rev` listában szereplő `{{r,c}, v}` párok által meghatározott pozíciókba a megfelelő `v` kerül, a többi cella 0. 
  - `n = 2`, `pairs_rev = [{{1, 2}, 5}]` -> `[[0, 5], [0, 0]]`
  - `n = 3`, `pairs_rev = [{{1, 1}, 1}, {{2, 3}, 2}, {{3, 2}, 3}]` -> `[[1, 0, 0], [0, 0, 2], [0, 3, 0]]`

- `feasible_line?/4` elvárt tulajdonság: Igaz akkor és csak akkor, ha a `{1..m}` halmazból hiányzó értékek száma nem több, mint a sor/oszlop hátralévő férőhelye.  
...
- Mérés: inputonként `Nhf1.helix/1` helyességének ellenőrzése és idő/memória profil
...
A futási idő és memóriahasználat drasztikusan nő a keresési térrel, ami összhangban van a DFS számításigényével. A memória igény is jelentős a 10. tesztesetben. Az összes kiadott tesztesetre helyes eredményt adott a függvény.